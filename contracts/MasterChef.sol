...
contract MasterChef is Ownable {
        ...
        returns (uint256)
    {
        ...
        if (block.timestamp > pool.lastRewardTime && lpSupply != 0) {
            ...
            uint256 oxdReward =
                multiplier.mul(oxdPerSecond).mul(pool.allocPoint).div(
                    totalAllocPoint
                );
            accOXDPerShare = accOXDPerShare.add(
                oxdReward.mul(1e12).div(lpSupply)
            );
        }
    ...
    function updatePool(uint256 _pid) public {
        ...
        pool.accOXDPerShare = pool.accOXDPerShare.add(
            oxdReward.mul(1e12).div(lpSupply)
        );
        pool.lastRewardTime = block.timestamp;
    }
    ...
    function deposit(uint256 _pid, uint256 _amount) public {
        ...
        uint256 pending =
            user.amount.mul(pool.accOXDPerShare).div(1e12).sub(user.rewardDebt);
